...
export const sharePlayerCardFlex = async (player: Player, team: Team, stats: any) => {
    ...
    try { await window.liff.shareTargetPicker([flexMessage]); } catch (error) { alert("ไม่สามารถแชร์ได้ (กรุณาเปิดใน LINE)"); }
};
...
export const shareMatch = async (match: Match, teamAName: string, teamBName: string, teamALogo: string, teamBLogo: string) => {
  ...
  if (isFinished && match.kicks) { match.kicks.forEach(k => { if (k.result === KickResult.GOAL) { const playerName = k.player.split('(')[0].replace(/[#0-9]/g, '').trim(); if (k.teamId === 'A' || k.teamId === teamAName) scorersA.push(playerName); else if (k.teamId === 'B' || k.teamId === teamBName) scorersB.push(playerName); } }); }
